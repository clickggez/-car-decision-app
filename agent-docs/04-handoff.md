...
# 🖥️ ปรับหน้าเว็บ 4 ประเด็นตามที่ผู้ใช้เคาะ + พบว่าโมเดล FUEL ใช้ฟีเจอร์จริงแค่ 5 จาก 26

## ⭐ ของสำคัญที่สุดของรอบนี้

`fuel_model.pkl` ที่ deploy อยู่ **ใช้ฟีเจอร์จริงแค่ 5 คอลัมน์จาก 26** ที่ส่งเข้าไป
`ColumnTransformer` ตั้ง `remainder='drop'` ทิ้ง 21 คอลัมน์ทิ้งก่อนเข้าโมเดล
...
ทำซ้ำได้ด้วย `python analysis/fuel_feature_usage_2026-09-23.py` → `fuel_feature_usage_2026-09-23.txt`
permutation test (10 seeds): สลับ 21 ฟีเจอร์ที่ถูก drop พร้อมกัน **คำตอบเปลี่ยน 0.0% ทุก seed**
ส่วน nep_score สลับแล้วเปลี่ยน **20.5% (ช่วง 18.7–22.8)**

⚠️ **ข้อจำกัดที่ต้องพูดทุกครั้งที่อ้างเลขนี้** (codex ค้านไว้ถูกใน `board.json` #28)
เป็น **in-sample sensitivity** บนข้อมูล 316 แถวชุดเดิม ไม่ใช่ out-of-sample และไม่ใช่ accuracy
**ห้ามเอา 0.4842/0.4810 ไปแทนเลข 0.4180** ที่วัดด้วย 20 splits ใน `verify_web_accuracy.py`

`buy_model.pkl` ไม่มีปัญหานี้ — ใช้ครบ 27 ฟีเจอร์ (num 10 + cat 17)

## สิ่งที่ทำ (ผู้ใช้เคาะครบทั้ง 4 ข้อก่อนลงมือ)

**1. หน้าแรก + หน้าล็อกอิน** — เปลี่ยนชื่อระบบเป็น "ระบบสนับสนุนการตัดสินใจเลือกซื้อรถยนต์ตามประเภทเชื้อเพลิง"
ตัด `Data Mining · SVM · ANN`, "เขตบางขุนเทียน", ป้าย EV|Hybrid|ICE และ**ตัด section "เทคโนโลยีที่ใช้" ทั้งบล็อก**
(ผู้ใช้สั่ง "ตัดออกให้หมด") · แก้ขนาดหัวข้อไม่ให้ล้นขอบจอมือถือ (`clamp(22px, 4.4vw, 38px)` + `<wbr>`)

**2. ฟอร์ม `predict_fuel` ตัดคำถามที่โมเดลไม่ได้ใช้ออก 7 ข้อ** (19 ช่อง → 12 ช่อง)
ตัด: `usage_type`, `frequency`, `distance`, `tech_env_concern`, `resale_maintenance_concern`, `ev_exposure`, `range_anxiety`
เหลือ: `prev_car` · priority (10 ตัวเลือก) · NEP 5 ข้อ — ทั้งหมดคือฟีเจอร์ที่โมเดลใช้จริง
`feature_encoding.fuel_features_from_web` เติมค่า default ให้ครบอยู่แล้ว (`:170-175`, `:237-240`) **โมเดลจึงยังรับ input ครบ 26 คอลัมน์เท่าเดิม ไม่ต้องแก้ encoder และไม่ต้องเทรนใหม่**
`validators.py` ย้าย 7 ฟิลด์ไป `_FUEL_OPTIONAL_WHITELISTS` — ไม่บังคับกรอก แต่ถ้าส่งมาต้องเป็นค่าที่ถูกต้อง

⚠️ **ข้อมูลวิจัย 500 ชุดเดิมไม่ถูกแตะ** ตามที่ผู้ใช้สั่ง ("ตัดออกในหน้าเว็บ แต่ข้อมูลวิจัยยังต้องเก็บไว้")
แต่ต้องรู้ว่า **ตั้งแต่นี้ไปจะไม่ได้เก็บคำตอบ 7 ข้อนั้นจากผู้ใช้ใหม่อีก**

**3. แก้บั๊ก dashboard แสดงผลของคนอื่น** (บั๊กร้ายแรงสุดในรอบนี้)
เดิม `dashboard.html:49-51,261` ฝัง "EV 78% ANN (mock)" ไว้ตรง ๆ และ `app.py` เรนเดอร์หน้าเปล่าไม่ส่ง session เข้าไป
ตอนนี้ `dashboard()` ส่ง `fuel=session['fuel_prediction']` / `buy=session['buy_prediction']` เข้า template
ยังไม่เคยวิเคราะห์ = แสดงกล่อง "ยังไม่มีผลวิเคราะห์" พร้อมปุ่มเริ่มทำแบบประเมิน **ไม่เติมตัวเลขให้อีกต่อไป**
ตัด score card 4 ใบ (92/75/88/60) ทิ้ง — เป็นค่าที่ไม่มีที่มาจากโมเดลเลย
`/api/dashboard` เลิกคืน `source='mock'` เปลี่ยนเป็น `'session'` หรือ `'empty'` (codex ชี้จุดนี้ไว้ใน #28)

**4. ถอด login ฝั่งผู้ใช้ → guest session**
เพิ่ม decorator `session_required` ใน `app.py` ออก `guest_<random12>` ให้อัตโนมัติ
แทน `@login_required` ที่ 9 route ฝั่งผู้ใช้ · **`admin_required` ไม่แตะเลย** (แอดมิน 8 route ยังต้องล็อกอินเหมือนเดิม)
guest **ไม่มีสิทธิ์แอดมิน** (`is_admin` ไม่ถูกตั้งใน decorator นี้) และผลวิเคราะห์ยังบันทึกเข้า Firebase ด้วย uid ของ guest
ตามที่ผู้ใช้เคาะ (ข้อ 3: "เก็บ") จึงยังแยกคนได้ · ถ้าล็อกอินด้วยบัญชีจริง uid เดิมไม่ถูกทับ

## ผลทดสอบ

**pytest 42/42 ผ่าน** (เดิม 41 ตัว — เพิ่ม 1 ตัว)
เทสต์ 5 ตัวที่ยืนยันพฤติกรรมเก่าถูก**เขียนใหม่ให้ยืนยันพฤติกรรมใหม่** ไม่ได้ลบทิ้ง:
- `test_dashboard_api_source_is_never_mock` / `test_dashboard_api_returns_empty_when_no_result` / `test_dashboard_returns_real_session_result`
- `test_guest_not_redirected_to_login` / `test_guest_gets_own_uid_and_no_admin_rights` / `test_guest_not_sent_to_login_but_must_pass_buy_step`

**ทดสอบ flow จริงบนเซิร์ฟเวอร์** (ไม่ล็อกอินเลย)
guest → กรอก buy → ได้ "ซื้อ" → กรอก fuel (12 ช่อง) → ได้ "สันดาป (ICE)" scores EV 26 / Hybrid 36 / ICE 38
→ dashboard แสดง **"ผลลัพธ์: คุณเหมาะสมกับ สันดาป (ICE) · ความมั่นใจ 38.0% · BAGGING (real)"** ตรงกับผลจริง
ไม่เหลือ "78.0%" ในหน้าแล้ว · uid ที่ได้คือ `guest_884eb0c73f16`

## ⚠️ ของค้างที่ต้องรู้

1. **สงสัยว่ามี horizontal overflow บนจอแคบ** — แคปหน้าจอที่ viewport 430px แล้วเนื้อหาถูกตัดขอบขวาทั้ง `/` และ `/predict/buy`
   แต่ที่ 800px ปกติดี **ยังไม่ฟันธงว่าเป็นบั๊กจริง** เพราะ Chrome headless ไม่ได้จำลอง mobile จริง (ไม่มี device emulation)
   ต้องเปิดบนมือถือจริงหรือ DevTools device mode ยืนยันก่อน — **ยังไม่ได้แก้ อย่าเพิ่งไปแก้ CSS ตามภาพ headless**
2. `dashboard.html` ยังมีกราฟค่าใช้จ่าย/เบี้ยประกันที่เป็นข้อมูลอ้างอิงทั่วไป (ไม่ใช่ของผู้ใช้) — มีป้าย "* ข้อมูลจำลอง" กำกับอยู่แล้ว จึงคงไว้
   ข้อความใต้กราฟยังเขียนว่า "เขตบางขุนเทียน" (ผู้ใช้สั่งตัดเฉพาะหน้าแรก ไม่ได้สั่งทั่วเว็บ)
3. `recommend.html:40` ยังมีป้าย Mock — ไม่ได้แตะรอบนี้

## ไม่ได้แตะ

`train_models.py` · `models/*.pkl` · `data/cars.json` · `firebase*` · `predict_buy.html` (ฟอร์ม BUY ใช้ฟีเจอร์ครบ 27 ห้ามตัด)

## ประชุมที่เกี่ยวข้อง

`agent-docs/meeting/board.json` #25–#29 หัวข้อ "ปรับปรุงหน้าตาเว็บและชุดคำถาม"
antigravity เปิดประเด็น (#25) · claude ตรวจไฟล์จริง (#26) · **codex ตรวจแย้งกลับ (#27, #28)** · claude รับข้อค้านและแก้ (#29)
codex รอบนี้ **โพสต์เข้าห้องประชุมเองได้แล้ว** (ข้อจำกัดเดิมหมดไปที่ codex-cli 0.155 — ดู #24)
...
# 🧪 ลองดันความแม่นยำครบทุกวิธีที่เหลือ — ไม่ขึ้น (หลักฐานชิ้นที่ 18) + พบเลขเอกสารไม่ตรงโมเดลจริง

## ที่มา

ผู้ใช้สั่ง "ทำยังไงก็ได้ให้ % สูงขึ้น" ทั้งที่ `00-READ-FIRST.md:11` เขียนว่า
**"งานปรับปรุงความแม่นยำของโมเดลปิดแล้ว"** — แจ้งความขัดแย้งให้ผู้ใช้ทราบแล้ว
**ผู้ใช้ยืนยันให้ทำ** จึงเดินหน้า (ไม่ใช่การข้ามกฎเอง)

## สิ่งที่ทำ

**1. ประชุม AI 3 ตัว** หัวข้อ "ดันความแม่นยำโมเดลให้สูงขึ้น" — ข้อความ **#17–#22** ใน
`agent-docs/meeting/board.json` เข้าร่วมครบทั้ง antigravity (#18) และ codex (#20 — Claude โพสต์แทน
ตามข้อจำกัดเดิมที่ `codex exec` เรียก MCP เองไม่ได้)

**2. ลองจริง 5 วิธีที่ยังไม่เคยลอง** ติดตั้ง `catboost 1.2.10` + `lightgbm 4.7.0` เพิ่มในเครื่อง
สคริปต์ `analysis/push_accuracy_2026-09-15.py` → ผล `analysis/push_accuracy_2026-09-15.txt`
**ตั้งเกณฑ์รับ/ไม่รับไว้ก่อนรัน** (ล็อกไว้ในหัวไฟล์ผล) · ทุก arm fit **เฉพาะ train fold**
· ใช้ split ชุดเดียวกันทุก arm (StratifiedShuffleSplit n=20, test 20%, seed 42)

**BUY** — CTRL (RF + BorderlineSMOTE) = 0.6805

| arm | accuracy | ต่าง CTRL | p (paired) |
|---|---|---|---|
| A1 SMOTE-Tomek | 0.6885 ± 0.0449 | +0.0080 | 0.4090 |
| A2 SMOTE-ENN | 0.6635 ± 0.0407 | −0.0170 | 0.0857 |
| A3 tree-select + RF | 0.6750 ± 0.0322 | −0.0055 | 0.3938 |
| A4 CatBoost | 0.6830 ± 0.0385 | +0.0025 | 0.8105 |
| A5 LightGBM | 0.6635 ± 0.0360 | −0.0170 | **0.0212 → แย่ลงอย่างมีนัยสำคัญ** |

**FUEL** — CTRL = 0.4359 · A1 เสมอเป๊ะ (+0.0000, p=1.0000) ที่เหลือแย่ลงทุกตัว
🔴 **A2 SMOTE-ENN พังหนัก 0.2414 (−0.1945, p=0.0000)**

➡️ **ไม่มี arm ไหนผ่านเกณฑ์แม้แต่ตัวเดียว** ทั้ง BUY และ FUEL = หลักฐานชิ้นที่ 18 ของเพดานสารสนเทศ

**3. สคริปต์หลักฐานใหม่** `analysis/verify_web_accuracy.py` → `analysis/verify_web_accuracy_2026-09-16.txt`
วัดความแม่นยำของ `.pkl` ที่เว็บใช้อยู่จริง**ใหม่ทั้งหมด** (ไม่เชื่อเลขในคีย์ `metrics`) พร้อม SHA-256

| | วัดใหม่ | เลขที่ฝังใน .pkl | ผล |
|---|---|---|---|
| BUY (n=500) | 0.6900 ± 0.0359 | 0.69 ± 0.0359 | **ตรงกัน** (ส่วนต่าง +0.0000) |
| FUEL (n=316) | 0.4180 ± 0.0548 | 0.418 ± 0.0548 | **ตรงกัน** (ส่วนต่าง −0.0000) |

SHA-256 ของไฟล์โมเดลที่วัด (ลายนิ้วมือ พิสูจน์ว่าวัดตัวเดียวกับที่เว็บโหลด)
- `buy_model.pkl` · `f6d7eefd201f461d9386d0d56ad54ad2189bdf244076a547a561e9fe3d0d9a28`
- `fuel_model.pkl` · `7eaeadebcc2ff6ebfdbf510fa4cdb5f11308be3759c1dfa542c95c370f6eb7ce`

kappa ที่วัดใหม่: BUY **+0.3462 ± 0.0761** · FUEL **+0.1163 ± 0.0723**
**ไฟล์นี้ใช้แนบภาคผนวกวิทยานิพนธ์ได้** ใครก็รันซ้ำได้: `python analysis/verify_web_accuracy.py`

## ⚠️ ต้องรู้

🔴 **เลขใน `00-READ-FIRST.md` ข้อ 1 ไม่ตรงกับ `.pkl` ที่ deploy จริง**

| | เอกสารเดิม (ชุด balanced_subsample) | `.pkl` ที่เว็บใช้ | ต่างกัน |
|---|---|---|---|
| BUY | 0.6925 ± 0.0391 | 0.6900 ± 0.0359 | **0.25 จุด** |
| FUEL | 0.4336 ± 0.0570 | 0.4180 ± 0.0548 | **1.56 จุด** ← ต่างมากกว่า ~6 เท่า |

**แก้แล้วในรอบนี้ ดูตารางใหม่ในข้อ 1**

⚠️ **FUEL มีเลขหลายชุดปนอยู่ในระบบ** — 0.4336 (เต็ม) · 0.4180 (จาก `.pkl`) · 0.4035 (กลุ่มสมดุล)
· 0.4133 (ชุดเก่า 2026-08-02) · 0.4433 (selective@70%) · 0.5461 (ยุบ 2 คลาส)
**การหยิบชุดที่สวยที่สุดไปเขียนเล่มคือ outcome switching** เป็นจุดที่กรรมการจับได้ง่ายที่สุด

## ข้อสรุปที่ผู้ใช้เคาะแล้ว

1. เวลาพูดถึง **"ระบบ"** ให้ใช้เลขจาก `.pkl` เป็นหลัก · เลขชุด `balanced_subsample`
   **เก็บไว้ ไม่ลบ** แต่ต้องติดป้ายกำกับว่าเป็น **คนละโปรโตคอล**
2. **ไม่ทำ** selective classification เป็นโค้ด → ให้เขียนเป็น **กราฟ trade-off ในบทอภิปราย** แทน
3. ยืนยันแล้วว่า **`BaggingClassifier` deploy อยู่จริง** ทั้งสองเป้า (งานที่อาจารย์สั่งจบแล้ว)

## ไม่ได้แตะ

โค้ดเว็บ · `train_models.py` · `.pkl` ใด ๆ
รอบนี้แตะแค่ `analysis/` (เพิ่มไฟล์ใหม่ 4 ไฟล์) กับ `agent-docs/` เท่านั้น
...
- **tests 41/41 ผ่านแล้ว** — เดิมเครื่องนี้ไม่มี pytest เลย (`No module named pytest`)
  ติดตั้ง `pytest 9.1.1` เพิ่มให้แล้ว (ไม่มี venv ลงที่ Python 3.13 หลักของเครื่อง)
  ตรวจเพิ่มด้วยการเรนเดอร์จริงผ่าน Flask test client ได้ 200 ทุกหน้า
  ⚠️ `pytest` ยังไม่ได้ใส่ใน `requirements.txt` — ถ้าจะให้เครื่องอื่นรันเทสได้ต้องเพิ่มเอง
...
1. เปิดเว็บดูของจริงด้วยตา (โดยเฉพาะหน้า `/recommend` ลองสลับแท็บ และ dashboard ลองสลับธีม) — เทสต์ผ่านแล้วแต่ยังไม่มีใครดูด้วยตา
