...
### Philosophy
Dark theme follows Tesla's same minimalist philosophy — swap the canvas from white to near-black while keeping the single-accent-color discipline. Photography and product imagery remain the emotional anchor. The dark surface should feel like a premium showroom at night, not a developer tool.

### Dark Color Palette

| Role | Light Value | Dark Value | Notes |
|------|------------|------------|-------|
| Page Background | `#FFFFFF` | `#0D0D0D` | Near-black, avoids pure #000 to reduce eye strain |
| Surface / Card | `#FFFFFF` | `#171A20` | Carbon Dark becomes the primary surface |
| Alternate Surface | `#F4F4F4` | `#1E2127` | Slightly lighter than surface for section differentiation |
| Primary Text | `#171A20` | `#E8E8E8` | High contrast but not pure white to reduce glare |
| Secondary Text | `#393C41` | `#B0B3B8` | Softer gray for body copy |
| Tertiary Text | `#5C5E62` | `#8A8D91` | Muted for sub-links and hints |
| Placeholder Text | `#8E8E8E` | `#5C5E62` | Dimmer in dark context |
| Border / Divider | `#EEEEEE` | `#2D2F33` | Subtle separation lines |
| Primary CTA | `#3E6AE1` | `#3E6AE1` | Electric Blue stays identical — the brand anchor |
| Primary CTA Hover | — | `#5A82E8` | Slightly brighter blue on hover for visibility |
| Secondary Button BG | `#FFFFFF` | `#1E2127` | Dark surface with light text |
| Secondary Button Text | `#393C41` | `#E8E8E8` | Inverted from light theme |
| Frosted Glass Nav | `rgba(255,255,255,0.75)` | `rgba(13,13,13,0.80)` | Dark frosted glass with backdrop-filter |
| Overlay | `rgba(128,128,128,0.65)` | `rgba(0,0,0,0.70)` | Deeper overlay for modals |
...
:root {
  ...
  --border-color: #EEEEEE;
  --cta-blue: #3E6AE1;
  --cta-blue-hover: #3058C8;
  --btn-secondary-bg: #FFFFFF;
  --btn-secondary-text: #393C41;
  --nav-frosted: rgba(255, 255, 255, 0.75);
...
[data-theme="dark"] {
  --bg-primary: #0D0D0D;
  --bg-surface: #171A20;
  --bg-surface-alt: #1E2127;
  --text-primary: #E8E8E8;
  --text-secondary: #B0B3B8;
  --text-tertiary: #8A8D91;
  --text-placeholder: #5C5E62;
  --border-color: #2D2F33;
  --cta-blue: #3E6AE1;
  --cta-blue-hover: #5A82E8;
  --btn-secondary-bg: #1E2127;
  --btn-secondary-text: #E8E8E8;
  --nav-frosted: rgba(13, 13, 13, 0.80);
...
- Respect `prefers-color-scheme: dark` media query as default when no stored preference
- Toggle transition: `background-color 0.33s, color 0.33s` on `body` to match Tesla's universal 0.33s timing

### Dark Theme Do's and Don'ts

**Do:**
- Keep Electric Blue (`#3E6AE1`) as the sole accent color — it works on both themes
- Use `#0D0D0D` not pure `#000000` — pure black causes OLED smearing and harsh contrast
- Maintain the same typography weights (400/500) — no weight adjustments for dark mode
- Let hero photography remain full-bleed with no added overlays
- Ensure 4.5:1 minimum contrast ratio for all text on dark surfaces (WCAG AA)

**Don't:**
- Invert shadows — dark theme should remain flat and shadow-free like light theme
- Add glowing effects or colored borders to compensate for dark backgrounds
- Change the CTA blue color between themes — brand consistency is non-negotiable
- Use pure white (`#FFFFFF`) for body text — `#E8E8E8` reduces eye strain on dark backgrounds
- Add extra decoration in dark mode — the same "radical subtraction" philosophy applies
